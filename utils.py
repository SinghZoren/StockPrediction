...
def create_stock_plot(canvas_widget, y_train, train_pred, y_test, test_pred, ticker, r2_score, dates):
    ...
    ax1.plot(train_dates, y_train, label='Training (Actual)', alpha=0.7, color='blue', linewidth=2)
    ax1.plot(train_dates, train_pred, label='Training (Predicted)', alpha=0.7, color='lightblue', linewidth=2)
    ax1.plot(test_dates, y_test, label='Testing (Actual)', alpha=0.7, color='green', linewidth=2)
    ax1.plot(test_dates, test_pred, label='Testing (Predicted)', alpha=0.7, color='lightgreen', linewidth=2)
    ...
    # Add legend to top right of the plot
    ax1.legend(loc='upper right', fontsize=8, bbox_to_anchor=(0.99, 0.99))
    ...
    if len(recent_train_dates) > 0:
        ax2.plot(recent_train_dates, recent_train_actual, label='Last Week (Actual)', 
                alpha=0.7, color='blue', linewidth=2)
        ax2.plot(recent_train_dates, recent_train_pred, label='Last Week (Predicted)', 
                alpha=0.7, color='lightblue', linewidth=2)
    
    ax2.plot(test_dates, y_test, label='Testing (Actual)', 
            alpha=0.7, color='green', linewidth=2)
    ax2.plot(test_dates, test_pred, label='Testing (Predicted)', 
    ...
    # Add legend to top right of the detail plot
    ax2.legend(loc='upper right', fontsize=8, bbox_to_anchor=(0.99, 0.99))
    ...
    # Position the performance text box in the bottom left
    ...
    plt.subplots_adjust(right=0.95, left=0.1, bottom=0.15, top=0.95, hspace=0.3)
